...
const SNAPSHOT_COLUMNS: Column<Record<string, unknown>>[] = [
  ...
  { key: 'savings_dbus', label: 'Savings (Credits)', format: formatCredits, align: 'right' },
...
const TIMESERIES_COLUMNS_BASE: Column<Record<string, unknown>>[] = [
  ...
  { key: 'name', label: 'Customer' },
  { key: 'contract_type', label: 'Contract Type' },
  { key: 'savings_dbus', label: 'Savings (Credits)', format: formatCredits2, align: 'right' },
...
export default function KWOSnowflakePage() {
  ...
  const fetchSnapshot = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/kwo-snowflake/snapshot?${buildParams()}`)
      if (!res.ok) {
  ...
  return (
    <div className="flex flex-col gap-6 p-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-xl font-semibold text-foreground font-heading">KWO for Snowflake</h1>
          <p className="text-sm text-muted-foreground mt-0.5">Keebo Warehouse Optimization — Snowflake</p>
        </div>
        {dataAsOf && (
          ...
          {noCustomers ? (
            <>
              ...
              <div>
                ...
                <DataTable columns={SNAPSHOT_COLUMNS} rows={[]} defaultSortKey="savings_dbus" csvFilename="kwo_snowflake_snapshot.csv" />
              </div>
            </>
          ) : (
            <>
                ...
                <>
                  ...
                  {snapshot.kpis ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                      <KPITile label="Savings" unit="Credits" value={snapshot.kpis.savings_dbus} delta={snapshot.kpis.delta_savings_dbus} absoluteDelta={snapshot.kpis.abs_delta_savings_dbus} format="dbu" higherIsBetter={true} />
                      ...
                      <KPITile label="Optimization Paused Spend" unit="Credits" value={snapshot.kpis.paused_spend_dbus} delta={snapshot.kpis.delta_paused_spend_dbus} absoluteDelta={snapshot.kpis.abs_delta_paused_spend_dbus} format="dbu" higherIsBetter={false} />
                      <KPITile label="Total Spend" unit="Credits" value={snapshot.kpis.total_spend_dbus} delta={snapshot.kpis.delta_total_spend_dbus} absoluteDelta={snapshot.kpis.abs_delta_total_spend_dbus} format="dbu" higherIsBetter={false} />
                      ...
                      <KPITile label="Auto-suspend Optimizations" unit="Count" value={snapshot.kpis.auto_stop_optimizations} delta={snapshot.kpis.delta_auto_stop_optimizations} absoluteDelta={snapshot.kpis.abs_delta_auto_stop_optimizations} format="count" higherIsBetter={true} />
                    </div>
                  ) : (
                  ...
                  <div>
                    ...
                    <DataTable
                      ...
                      csvFilename="kwo_snowflake_snapshot.csv"
                    />
                  </div>
                </>
              )}
          ...
          {noCustomers ? (
            <>
              ...
              <div>
                ...
                <DataTable columns={TIMESERIES_COLUMNS} rows={[]} defaultSortKey="period_start" csvFilename="kwo_snowflake_timeseries.csv" />
              </div>
            </>
          ) : (
            <>
                ...
                <>
                  ...
                  <div>
                    ...
                    <DataTable
                      ...
                      csvFilename="kwo_snowflake_timeseries.csv"
                    />
                  </div>
                </>
              )}
