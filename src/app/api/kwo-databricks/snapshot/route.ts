...
export async function GET(req: NextRequest) {
  try {
    ...
    const allOrgIds = getOrgIdsForContractTypes(contractTypes, priorStartStr, weekEndStr)
    ...
    const currentContractTypeMap = new Map(
      orgIds.map((id) => [id, getContractTypeForOrgInRange(id, weekStartStr, weekEndStr) ?? 'consumption'])
    )
    const priorContractTypeMap = new Map(
      orgIds.map((id) => [id, getContractTypeForOrgInRange(id, priorStartStr, priorEndStr) ?? 'consumption'])
    )
