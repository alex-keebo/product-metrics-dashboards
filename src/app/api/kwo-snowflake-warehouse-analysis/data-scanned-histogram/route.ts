...
import type { DataScannedHistogramBucket, DataScannedHistogramResponse } from '@/lib/types'
...
export async function GET(request: NextRequest) {
  ...
  try {
    const sqlTemplate = fs.readFileSync(
      path.join(process.cwd(), 'sql', 'kwo_snowflake_warehouse_data_scanned_histogram.sql'),
      'utf-8'
    )
    ...
    const buckets: DataScannedHistogramBucket[] = rows
      .sort((a, b) => a.bucket_order - b.bucket_order)
      .map((r) => ({ bucket_label: r.bucket_label, query_count: Number(r.query_count) }))

    const response: DataScannedHistogramResponse = { buckets }
    return NextResponse.json(response)
  } catch (err) {
    console.error('[snf-warehouse-data-scanned-histogram]', err)
