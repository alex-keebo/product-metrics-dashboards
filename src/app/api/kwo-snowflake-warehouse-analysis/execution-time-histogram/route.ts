...
import type { ExecutionTimeHistogramBucket, ExecutionTimeHistogramResponse } from '@/lib/types'
...
export async function GET(request: NextRequest) {
  ...
  try {
    const sqlTemplate = fs.readFileSync(
      path.join(process.cwd(), 'sql', 'kwo_snowflake_warehouse_execution_time_histogram.sql'),
      'utf-8'
    )
    ...
    const buckets: ExecutionTimeHistogramBucket[] = rows
      .sort((a, b) => a.bucket_order - b.bucket_order)
      .map((r) => ({ bucket_label: r.bucket_label, query_count: Number(r.query_count) }))

    const response: ExecutionTimeHistogramResponse = { buckets }
    return NextResponse.json(response)
  } catch (err) {
    console.error('[snf-warehouse-execution-time-histogram]', err)
