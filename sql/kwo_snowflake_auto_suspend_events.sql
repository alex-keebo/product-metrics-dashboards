-- KWO for Snowflake: Auto-suspend optimization events
--
-- Parameters:
--   @start_date DATE          Start of the date range (inclusive)
--   @end_date   DATE          End of the date range (inclusive)
--   @org_ids    ARRAY<STRING> List of org_ids to include
--
-- Counts Keebo-driven auto-suspend events per org per day.
-- Excludes events where state = 'FAILED' (no effective optimization occurred).
-- NULL state is treated as successful (not failed).

SELECT
  DATE(ts) AS date,
  org_id,
  COUNT(*)         AS event_count
FROM `keebo-portal.federated_views_tf.alter_auto_suspend`
WHERE DATE(ts) BETWEEN @start_date AND @end_date
  AND org_id IN UNNEST(@org_ids)
  AND (state != 'FAILED' OR state IS NULL)
GROUP BY date, org_id
ORDER BY date, org_id
