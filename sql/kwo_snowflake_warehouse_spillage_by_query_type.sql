...
--
-- Total spilled bytes (local + remote) per query_type across the full
-- requested date range. Ranking/top-10+Other collapsing happens in the API route.

SELECT
  query_type,
  SUM(IFNULL(bytes_spilled_to_local_storage, 0) + IFNULL(bytes_spilled_to_remote_storage, 0)) AS metric_value
